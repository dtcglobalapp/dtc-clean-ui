...
async function loadDashboard() {
  ...
  try {
    ...
    const expiringSoon = docs.filter(
      (doc) => doc.expires_at && doc.expires_at >= now && doc.expires_at <= soon
    ).length;
    ...
    if (pending > 0) {
      alerts.push({
        ...
        message:
          "Review and approve uploaded files so the daycare stays organized and inspection-ready.",
        tone: "warning",
      });
    }
    ...
    if (expiringSoon > 0) {
      alerts.push({
        ...
        message:
          "Follow up before deadlines arrive, avoid stress, and keep records updated for the city and agencies.",
        tone: "warning",
      });
    }
    ...
    if (totalChildren > 0 && totalDocuments === 0) {
      alerts.push({
        ...
        message:
          "Start uploading vaccine records, medical forms, agreements, and other required files.",
        tone: "neutral",
      });
    }
    ...
    if (!alerts.length) {
      alerts.push({
        ...
        message:
          "Your daycare dashboard is active and ready. As more modules grow, DTC will surface smarter operational alerts here.",
        tone: "success",
      });
    }
